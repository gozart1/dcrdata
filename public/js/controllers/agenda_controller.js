...
export default class extends Controller {
  ...
  async connect () {
    ...
    this.drawCharts()
    let response = await axios.get('/api/agenda/' + this.agendaId)
    this.cumulativeVoteChoicesChart.updateOptions({
      file: cumulativeVoteChoicesData(response.data.by_time)
    })
    this.voteChoicesByBlockChart.updateOptions({
      file: voteChoicesByBlockData(response.data.by_height)
    })
